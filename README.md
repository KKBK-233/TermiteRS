...
原有命令保持兼容。助理窗口现在也接受“持续盯着我的 D9 PR，CI 或审查状态变化时记录下来，每十分钟检查一次”这样的自然语言。模型只生成受限任务计划，Rust 核心校验 GitHub 范围、间隔并持久化；窗口保持打开时，调度器会自动执行到期任务。状态变化会进入只读评估队列，使用 `/watch-decisions` 查看；需要改代码、推送、回复、合并或写 Linear 的建议一定标为“待判断”。可使用 `/watch-pause <名称>` 和 `/watch-resume <名称>` 控制任务，不会写入 GitHub 或触发分支同步。
