...
use super::{CargoPackageCoordinate, CargoReachabilitySnapshot, StaticIndicator, StaticScanReport};
...
"#,
    ...
    fn cached_crate_is_scanned_without_executing_build_script() {
        let root =
            std::env::temp_dir().join(format!("termiters-crate-evidence-{}", Uuid::new_v4()));
...
"#
    ...
    fn live_current_lock_can_be_acquired_without_building_project() {
        ...
        let report = scan_locked_cargo_dependencies(
            "TermiteRS",
